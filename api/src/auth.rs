...
impl Token {
    fn create(account_id: i32, secret_key: &str) -> JWTResult<String> {
    ...
    fn validate(token: &str, secret_key: &str) -> Result<i32, core::num::ParseIntError> {
...
impl<'r> FromRequest<'r> for User {
    ...
    async fn from_request(req: &'r Request<'_>) -> Outcome<User, Self::Error> {
        ...
        match Token::validate(&token, &config.secret_key) {
...
async fn login(
    ...
    login: Json<LoginInfo>,
) -> Option<Json<TokenOut>> {
        ...
        .unwrap()
    {
        Some(account_object) => Some(Json(TokenOut {
            token: Token::create(account_object.id, &config.secret_key).unwrap(),
        })),
        None => None,
    }
}
